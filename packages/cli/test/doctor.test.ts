...
void test("exported launchers are executable and delegate unchanged arguments", async () => {
  ...
  assert.match(launcher, /import\.meta\.resolve\("@piew\/cli"\)/);
  assert.match(launcher, /@piew\/cli/);
  assert.match(output, /Exported .*cli-echo/);
  assert.match(warning, /not in PATH/);

  const packageRoot = join(paths.agentDir, "npm", "node_modules", "@piew/cli");
  const fallbackCli = join(packageRoot, "dist", "src");
  const indexUrl = pathToFileURL(join(process.cwd(), "../core", "dist", "src", "index.js")).href;
  mkdirSync(fallbackCli, { recursive: true });
  writeFileSync(join(packageRoot, "package.json"), JSON.stringify({ name: "@piew/cli", version: "3.4.2" }));
...
void test("portable bundle export writes a self-contained payload and external-runtime launcher", async () => {
  ...
  const manifest = JSON.parse(readFileSync(join(destination, "manifest.json"), "utf8")) as { format: string; version: number; command: string; workflow: { name: string; input: unknown; output: unknown }; runtime: { pi: string; "@piew/cli": string }; requirements: { commands: string[]; environment: string[] } };
  ...
  assert.notEqual(manifest.runtime["@piew/cli"], "unknown");
  ...
  assert.match(readFileSync(join(destination, "payload", "runner.mjs"), "utf8"), /@piew\/cli@/);
...
void test("portable bundle setup resolves an external runtime, launches, and fails closed on requirements", () => {
  ...
  mkdirSync(join(agentDir, "npm", "node_modules", "@piew"), { recursive: true });
  mkdirSync(join(piRoot, "dist", "core", "tools"), { recursive: true });
  symlinkSync(process.cwd(), join(agentDir, "npm", "node_modules", "@piew/cli"));
  ...
  writeFileSync(join(agentDir, "settings.json"), JSON.stringify({ packages: ["npm:@piew/cli"] }));
