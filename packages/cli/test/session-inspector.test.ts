...
void test("session inspector uses the persisted script workflow identity", async () => {
  ...
  writeJsonl(parentPath, [
    { type: "session", version: 3, id: sessionId, timestamp: "2026-01-01T00:00:00.000Z", cwd },
    { type: "message", id: "parent-assistant", parentId: null, timestamp: "2026-01-01T00:00:01.000Z", message: { role: "assistant", content: [{ type: "toolCall", id: "call-registered", name: "workflow", arguments: { name: "registeredFunction", script: "return null;", args: {}, foreground: true } }], api: "openai-responses", provider: "provider-root", model: "model-root", usage: usage(0), stopReason: "toolUse", timestamp: 1 } },
