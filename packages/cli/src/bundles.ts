...
export interface PortableWorkflowManifest {
  ...
  runtime: { pi: string; "@piew/cli": string };
...
function runnerSource(): string {
  return [
    ...
    "  const roots = configured.filter((entry) => /^npm:@piew\\/cli(?:@|$)/.test(entry.source)).map((entry) => entry.installedPath);",
    "  roots.push(join(agentDir, 'npm', 'node_modules', '@piew', 'cli'));",
    ...
    "    if (typeof version === 'string' && satisfies(version, manifest.runtime['@piew/cli'])) return { root: realpathSync(root), version };",
    "  }",
    "  return undefined;",
    "}",
    "async function confirmInstall(pi, expected) {",
    "  const spec = `npm:@piew/cli@${installationVersion(expected)}`;",
    "  if (!(process.stdin.isTTY && process.stderr.isTTY)) throw new Error(`The compatible @piew/cli package is missing. Re-run '${manifest.command} setup --yes' to approve: ${pi} install ${spec}`);",
    ...
    "  const expected = manifest.runtime['@piew/cli'];",
    "  let engine = await findEngine(pi);",
    "  if (engine) return engine;",
    "  if (!allowInstall) throw new Error(`Compatible @piew/cli${expected === 'unknown' ? '' : `@${expected}`} is not installed through Pi. Run '${manifest.command} setup' first; no installation is performed during launch.`);",
    "  if (expected === 'unknown') throw new Error('The bundle does not record a compatible @piew/cli version. Re-export the bundle.');",
    "  if (!approve && !(await confirmInstall(pi, expected))) throw new Error('Installation was not approved.');",
    "  const spec = `npm:@piew/cli@${installationVersion(expected)}`;",
    "  if (spec.endsWith('@unknown')) throw new Error('The bundle does not record a compatible @piew/cli version. Re-export the bundle.');",
    ...
    "  if (!engine) throw new Error(`Pi installed an incompatible @piew/cli version; expected ${expected}.`);",
    "  return engine;",
    "}",
    ...
    "  if (state.format !== manifest.format || state.version !== manifest.version || typeof state.checkedAt !== 'string' || !satisfies(state.pi, manifest.runtime.pi) || !satisfies(state.engine, manifest.runtime['@piew/cli'])) throw new Error(`Bundle setup is missing or stale. Run '${manifest.command} setup' before launching.`);",
    "}",
    ...
    "  console.log('@piew/cli: ' + engine.version);",
    "}",
...
export function writePortableWorkflowBundle(input: PortableWorkflowBundleInput): PortableWorkflowManifest {
  ...
  const manifest: PortableWorkflowManifest = {
    ...
    runtime: { pi: input.piVersion?.trim() || "unknown", "@piew/cli": engineVersion.trim() || "unknown" },
    requirements: {
