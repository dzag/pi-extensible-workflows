...
function optionalRole(value: unknown, label: string): void {
  ...
  if (value.thinking !== undefined && value.thinking !== null && !["off", "minimal", "low", "medium", "high", "xhigh", "max"].includes(value.thinking as string)) throw new Error(`${label}.thinking is invalid`);
