...
void test("creates deterministic snapshot worktrees, preserves launch subdirectories, and cleans up only on confirmed deletion", async () => {
  ...
  const store = new RunStore(cwd, "session-a", "run-a", home);
  await store.create(run(cwd), snapshot);
  const first = await store.worktree("agent/path", undefined, true);
...
void test("leaves worktrees dirty without an explicit commit opt-in and still reports them as changed", async () => {
  ...
  const store = new RunStore(cwd, "session-a", "run-a", home);
  await store.create(run(cwd), snapshot);
  const reference = await store.worktree("agent/path");
  assert.equal(reference.commit, undefined);
  assert.deepEqual(await store.changedWorktrees(), []);
  writeFileSync(join(reference.cwd, "agent.txt"), "post-creation");
  assert.equal(await store.snapshotWorktree("agent/path"), reference.base);
  assert.equal(execFileSync("git", ["-C", reference.path, "rev-parse", "HEAD"], { encoding: "utf8" }).trim(), reference.base);
  assert.equal(execFileSync("git", ["-C", reference.path, "status", "--porcelain"], { encoding: "utf8" }).trim(), "?? packages/app/agent.txt");
  assert.deepEqual(await store.changedWorktrees(), [reference]);
  await store.delete(true);
});
void test("creates snapshot worktrees at a caller-chosen location and rejects unusable locations", async () => {
  const home = mkdtempSync(join(tmpdir(), "pi-extensible-workflows-located-worktree-"));
  const repo = join(home, "repo");
  const cwd = join(repo, "packages", "app");
  mkdirSync(cwd, { recursive: true });
  execFileSync("git", ["init", "-q", repo]);
  ...
  const store = new RunStore(cwd, "session-a", "run-a", home);
  await store.create(run(cwd), snapshot);
  const absolute = join(home, "custom", "api");
  const located = await store.worktree("worktree/named/api", absolute);
  assert.equal(located.path, absolute);
  assert.equal(located.cwd, join(absolute, "packages", "app"));
  assert.equal(readFileSync(join(located.cwd, "tracked.txt"), "utf8"), "initial");
  assert.deepEqual(await store.worktree("worktree/named/api", absolute), located);
  assert.deepEqual(await store.validateWorktree("worktree/named/api"), located);
  assert.deepEqual(await store.validNamedWorktrees(), ["api"]);
  const relativeLocated = await store.worktree("worktree/named/ui", join("..", "..", "..", "relative-ui"));
  assert.equal(relativeLocated.path, join(home, "relative-ui"));
  await assert.rejects(store.worktree("worktree/named/api", join(home, "other")), (error: unknown) => error instanceof WorkflowError && error.code === "WORKTREE_FAILED" && error.message.includes("already exists at"));
  const nested = await store.worktree("worktree/named/inside", join(repo, "nested"));
  assert.equal(nested.path, join(repo, "nested"));
  await assert.rejects(store.worktree("worktree/named/parent", home), (error: unknown) => error instanceof WorkflowError && error.code === "WORKTREE_FAILED");
  writeFileSync(join(repo, ".gitignore"), ".worktrees/\n");
  execFileSync("git", ["-C", repo, "add", ".gitignore"]);
  execFileSync("git", ["-C", repo, "commit", "-qm", "ignore worktrees"]);
  const inRepo = join(repo, ".worktrees", "t1");
  const inRepoLocated = await store.worktree("worktree/named/inrepo", inRepo);
  assert.equal(inRepoLocated.path, inRepo);
  assert.equal(readFileSync(join(inRepoLocated.cwd, "tracked.txt"), "utf8"), "initial");
  // Gitignored in-repo worktrees stay invisible; non-ignored ones show up as untracked only.
  assert.equal(execFileSync("git", ["-C", repo, "status", "--porcelain"], { encoding: "utf8" }).trim(), "?? nested/");
  await store.delete(true);
  assert.equal(existsSync(absolute), false);
  assert.equal(existsSync(relativeLocated.path), false);
});
