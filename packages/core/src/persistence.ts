...
function decodeWorktreeReference(value: unknown): WorktreeReference | undefined {
  ...
  if (location === INVALID_PERSISTED_VALUE || (location !== undefined && !location.trim())) return undefined;
  if (value.commit !== undefined && typeof value.commit !== "boolean") return undefined;
  return { owner: value.owner, path: value.path, branch: value.branch, cwd: value.cwd, base: value.base, ...(location === undefined ? {} : { location }), ...(value.commit === undefined ? {} : { commit: value.commit }) };
}
...
export class RunStore {
  ...
  private structuralWorktree(owner: string, record: WorktreeReference): WorktreeReference {
    const expected = this.expectedWorktree(owner, record.location);
    // Custom locations live outside the run directory on purpose, so only default records are containment checked.
    const relativePath = record.location !== undefined ? "." : relative(this.directory, record.path);
