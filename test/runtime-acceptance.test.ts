...
void test("recovery inherits persisted launch mode for resume and retry", { timeout: 10000 }, async () => {
  ...
  const snapshot = (name: string, launchMode: "foreground" | "background") => createLaunchSnapshot({ script: `return ${JSON.stringify(name)};`, args: null, metadata: { name }, launchMode, settings: { concurrency: 1 }, models: ["openai/gpt"], tools: [], agentTypes: [], roles: {}, schemas: [] });
  ...
  for (let attempt = 0; attempt < 1000 && !messages.some((message) => message.startsWith("Workflow background-resume completed:")); attempt += 1) await new Promise<void>((resolve) => setTimeout(resolve, 1));
  ...
  assert.equal((await new RunStore(cwd, sessionId, childRunId, home).load()).run.state, "completed");
});
