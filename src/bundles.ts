...
function runnerSource(): string {
  return [
    ...
    "  const services = await agent.createAgentSessionServices({ cwd: process.cwd(), agentDir, modelRuntime, resourceLoaderOptions: { noPromptTemplates: true, noThemes: true, noContextFiles: true } });",
    ...
    "  const agentDir = process.env.PI_CODING_AGENT_DIR ?? join(homedir(), '.pi', 'agent');",
    "  return cli.runCli(['run', manifest.workflow.name, ...argv], { cwd: process.cwd(), agentDir, stderr: (text) => process.stderr.write(text) });",
    "}",
