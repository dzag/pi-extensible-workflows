...
function validateAgent(value: unknown, label: string): void { if (!object(value) || typeof value.id !== "string" || !value.id || typeof value.name !== "string" || !value.name || typeof value.path !== "string" || !value.path || typeof value.state !== "string" || !AGENT_STATES.has(value.state)) throw new Error(`${label} is invalid`); optionalString(value.systemPrompt, `${label}.systemPrompt`); optionalString(value.label, `${label}.label`); optionalString(value.parentId, `${label}.parentId`); if (value.structuralPath !== undefined) stringList(value.structuralPath, `${label}.structuralPath`); optionalString(value.parentBreadcrumb, `${label}.parentBreadcrumb`); optionalString(value.worktreeOwner, `${label}.worktreeOwner`); optionalString(value.role, `${label}.role`); optionalString(value.requestedModel, `${label}.requestedModel`); model(value.model, `${label}.model`); stringList(value.tools, `${label}.tools`); nonNegativeInteger(value.attempts, `${label}.attempts`); if (value.attemptDetails !== undefined) { if (!Array.isArray(value.attemptDetails)) throw new Error(`${label}.attemptDetails is invalid`); for (const [index, attempt] of value.attemptDetails.entries()) { const at = `${label}.attemptDetails[${String(index)}]`; if (!object(attempt) || !Number.isSafeInteger(attempt.attempt) || Number(attempt.attempt) < 1 || typeof attempt.transport !== "string" || !attempt.transport) throw new Error(`${at} is invalid`); if (attempt.session !== undefined && (!object(attempt.session) || typeof attempt.session.transport !== "string" || !attempt.session.transport || typeof attempt.session.sessionId !== "string" || !attempt.session.sessionId)) throw new Error(`${at}.session is invalid`); accounting(attempt.accounting, `${at}.accounting`); if (attempt.error !== undefined && (!object(attempt.error) || typeof attempt.error.code !== "string" || typeof attempt.error.message !== "string")) throw new Error(`${at}.error is invalid`); } } if (value.accounting !== undefined) accounting(value.accounting, `${label}.accounting`); }
...
function validateRunRecord(run: PersistedRun): void {
  ...
  if (!object(value) || typeof value.id !== "string" || !value.id || typeof value.workflowName !== "string" || !value.workflowName || typeof value.cwd !== "string" || !value.cwd || typeof value.sessionId !== "string" || !value.sessionId || !RUN_STATES.includes(value.state as RunState) || !Array.isArray(value.agents) || (!Array.isArray(value.agentSessions) && !Array.isArray(value.nativeSessions))) throw new Error("Persisted run state is invalid");
  ...
  for (const [index, session] of sessions.entries()) if (!object(session) || typeof session.transport !== "string" || !session.transport || typeof session.sessionId !== "string" || !session.sessionId) throw new Error(`agentSessions[${String(index)}] is invalid`);
