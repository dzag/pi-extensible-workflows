...
npx pi-extensible-workflows inspect [session-id] [--json|--summary] [--failed]
npx pi-extensible-workflows transcript <session-file>
npx pi-extensible-workflows run <workflow-name> [workflow arguments]
npx pi-extensible-workflows export <workflow-name> [--name <command>] [--output <path>] [--force] [--bundle]
npx pi-extensible-workflows bundle <workflow-name> [--name <command>] [--output <directory>] [--force]
```

`doctor` validates the installation and active Pi resources and remains read-only. `doctor cleanup` is an explicit, dry-run-first maintenance command: it previews terminal workflow runs older than 90 days across the current project's stored sessions and deletes them only with `--yes`; use `--older-than-days` to change the positive age threshold. `inspect` opens a read-only terminal view of persisted workflow runs. Use `inspect [session-id] --json` for machine-readable summaries or `--summary` for concise non-TTY output; add `--failed` to show only failed persisted runs (and to select concise output when no mode is given). These modes discover runs from persisted storage even when no transcript exists. Background workflow failures arrive as one-line human-readable follow-ups with the run ID, retry action, and artifact paths; structured failure details remain available to tool-result consumers. `transcript` renders a session transcript to stdout. `run` derives flat CLI arguments and help from a registered function's input schema. Use `--input '<json>'` for nested or otherwise complex inputs. It executes in the current working directory, writes the final JSON result to stdout, and writes progress, the completed run ID, and errors to stderr. `export` creates an executable POSIX launcher in `~/.local/bin` by default.
`bundle` creates a directory containing an executable launcher, compatibility `manifest.json`, and workflow payload. The bundle does not contain Pi or the workflow engine. Unpack it, run `<command> setup` to verify npm-installed Pi and approve installation of the pinned compatible `pi-extensible-workflows` package when needed, then run the command with workflow arguments. Use repeated `--role`, `--alias`, `--tool`, `--command`, and `--environment` options to declare recipient requirements; use `--extension`, `--skill`, `--resource`, and `--dependency` to include selected payload resources. Missing declared external commands, environment requirements, unavailable models, or unavailable tools fail before execution. Setup records a bundle-local state file and launch requires a current successful setup. `export --bundle` is an alias. `run` and `export` accept the trust overrides `--approve` and `--no-approve`; the generated launcher forwards its arguments to `run`. `--` ends launcher option parsing, and later tokens are passed to workflow input instead of being interpreted as launcher options. Headless `run`, generated `export` launchers, and bundles cannot execute workflows containing checkpoints; use the Pi workflow tool/UI path for checkpointed workflows.
